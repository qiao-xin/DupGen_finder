...
$out1=">$options{o}\/$options{t}.wgd.pairs-unique";
$out2=">$options{o}\/$options{t}.wgd.genes-unique";
$incl=1;
$wp=0;
$wg=0;
%wg=();
...
$out4=">$options{o}\/$options{t}.proximal.genes-unique";
open(output1,$out1);
open(output2,$out2);
open(output3,$out3);
open(output4,$out4);
...
if(!exists $wg{$a[0]} && !exists $wg{$a[1]})
{
...
if(!exists $wg{$a[0]} && !exists $wg{$a[1]})
{
	if(!exists $tg{$a[0]} && !exists $tg{$a[1]})
	{
...
$out1=">$options{o}\/$options{t}.transposed.pairs-unique";
$out2=">$options{o}\/$options{t}.transposed.genes-unique";
open(output1,$out1);
open(output2,$out2);
...
{
if(!exists $tg{$key} && !exists $pg{$key})
{
	print output2 "$key\t$gch{$key}\:$glc{$key}\n";
	$trg++;
}
...
$out5=">$options{o}\/$options{t}.dispersed.pairs-unique";
$out6=">$options{o}\/$options{t}.dispersed.genes-unique";
open(output5,$out5);
open(output6,$out6);
...
$out1=">$options{o}\/$options{t}.pairs.stats-unique";
open(output1,$out1);
